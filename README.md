...
- **하이브리드 미디어 갤러리**: 도서(알라딘)와 영화(TMDB)를 통합 렌더링. 홈(추천) 탭에서 태그 기반 개인화 큐레이션 제공
...
- **영감 발굴 (Discovery)**: 사용자의 관심 태그(도서 카테고리, 영화 장르)를 기반으로 알라딘/TMDB에서 추천 콘텐츠 큐레이션
- **AI 큐레이터**: Ollama(로컬 LLM) / Claude / Gemini를 활용한 태그 추천, 연관 도서/영화 추천
- **지식 그래프**: 도서, 영화, 메모에 사용된 태그들이 우주처럼 얽힌 인터랙티브 3D/2D 그래프
