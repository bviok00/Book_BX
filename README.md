...
- **영감 발굴 (Discovery)**: 사용자의 관심 태그(도서 카테고리, 영화/애니 장르)를 기반으로 알라딘/TMDB/AniList에서 추천 콘텐츠 큐레이션 (낙관적 업데이트 UX 적용으로 끊김 없는 탐색 지원)
- **상용 서비스 수준 UX 최적화**: 
  - `revalidateTag` 기반의 On-Demand Revalidation 캐싱 아키텍처
  - React Transition(`useTransition`)을 이용한 깜빡임 없는 Background Update
  - 넷플릭스 스타일 Skeleton UI 로더 적용
  - 하트 버튼(위시리스트) 즉각 반영을 위한 낙관적 업데이트(Optimistic Update)
...
2. **영감 발굴 큐레이션**: 첫 화면에서 자주 사용하는 태그를 바탕으로 새로운 책과 영화를 추천받으며, 흐름 끊김 없이 여러 작품을 연속으로 담을 수 있습니다.
