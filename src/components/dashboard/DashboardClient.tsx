...
export default function DashboardClient({
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      {currentTab === 'BOOK' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          {unifiedBooks.length > 0 && !bookFolderId && (!bookStatus || bookStatus === 'ALL') && (
            <DiscoverySection 
              existingIsbns={unifiedBooks.map(c => c.contentId)} 
              existingTmdbIds={unifiedMovies.map(c => c.contentId)}
              existingAnilistIds={unifiedAnimes.map(c => c.contentId)}
      ...
      {currentTab === 'MOVIE' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          {unifiedMovies.length > 0 && !movieFolderId && (!movieStatus || movieStatus === 'ALL') && (
            <DiscoverySection 
              existingIsbns={unifiedBooks.map(c => c.contentId)} 
              existingTmdbIds={unifiedMovies.map(c => c.contentId)}
              existingAnilistIds={unifiedAnimes.map(c => c.contentId)}
      ...
      {currentTab === 'ANIME' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          {unifiedAnimes.length > 0 && !animeFolderId && (!animeStatus || animeStatus === 'ALL') && (
            <DiscoverySection 
              existingIsbns={unifiedBooks.map(c => c.contentId)} 
              existingTmdbIds={unifiedMovies.map(c => c.contentId)}
              existingAnilistIds={unifiedAnimes.map(c => c.contentId)}
