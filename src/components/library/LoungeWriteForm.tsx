...
import { createLoungePost, getMyLibraryItems, updateLoungePost } from '@/app/dashboard/lounge-actions';
...
function LoungeWriteFormInner({ editId, initialData }: { editId?: string, initialData?: any }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [title, setTitle] = useState(initialData?.title || '');
  const [content, setContent] = useState(initialData?.content || '');
  const [isPreview, setIsPreview] = useState(false);
  const [mediaType, setMediaType] = useState<'BOOK' | 'MOVIE' | 'ANIME' | ''>(initialData?.media_type || '');
  
  // 선택된 컨텐츠 메타데이터
  const [mediaId, setMediaId] = useState<string>(initialData?.media_id || '');
  const [posterUrl, setPosterUrl] = useState<string>(initialData?.poster_url || '');
  const [contentTitle, setContentTitle] = useState<string>(initialData?.content_title || '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      let res;
      if (editId) {
        res = await updateLoungePost(editId, {
          title,
          content,
...
export default function LoungeWriteForm({ editId, initialData }: { editId?: string, initialData?: any }) {
  const router = useRouter();
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '24px 0' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 800, color: 'var(--text-primary)', margin: 0 }}>
          {editId ? '라운지 글 수정하기' : '새 라운지 글쓰기'}
        </h1>
        <button 
          onClick={() => router.back()}
      ...
      <Suspense fallback={<div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-tertiary)' }}>로딩 중...</div>}>
        <LoungeWriteFormInner editId={editId} initialData={initialData} />
      </Suspense>
    </div>
  );
}
